...
const size_t TOP_K_DEFAULT_EQ_DIST = 1000;
...
class TopKEqualDistinctCountHistogram : public AbstractHistogram<T> {
 public:
  ...
  TopKEqualDistinctCountHistogram(std::shared_ptr<AbstractHistogram<T>> histogram, std::vector<T>&& top_k_names, 
 ...
 private:
  ...
  // Pointer to the AbstractHistogram
