...
template <typename UntypedSegmentPosition>
std::vector<IndexRange> JoinIndex::_index_ranges_for_value(const UntypedSegmentPosition probe_side_position,
                                                           const std::shared_ptr<AbstractIndex>& index) const {
  std::vector<IndexRange> index_ranges{};
  index_ranges.reserve(2);

  // auto probe_side_position_value = probe_side_position.value();
  // if constexpr (std::is_same_v<UntypedSegmentPosition, SegmentPosition<pmr_string>>) {
  //   probe_side_position2 = std::string{probe_side_position};
  // }
  ...
  //   if (probe_side_position2.is_null() || indexed_null_values) {
  ...
  // if (!probe_side_position2.is_null()) {
  ...
  //       range_begin = index->lower_bound({probe_side_position2.value()});
  //       range_end = index->upper_bound({probe_side_position2.value()});
  //       break;
  //     }
  //     case PredicateCondition::NotEquals: {
  //       // first, get all values less than the search value
  //       range_begin = index->cbegin();
  //       range_end = index->lower_bound({probe_side_position2.value()});
  ...
  //       range_begin = index->upper_bound({probe_side_position2.value()});
